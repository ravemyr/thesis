function [u walltime]  = se2p_fourier_space(eval_idx,x,f,opt)
...
if use_kaiser
  ...
  pre_window = se2p_window_precomp(opt);
end
...
if use_kaiser
	pre_t = tic;
	S = se2p_precomp_kaiser(x,opt);
	walltime.pre = toc(pre_t);
	grid_fcn = @(f) SE_fg_grid_split_kaiser_mex_2p(x(S.perm,:),f(S.perm), ...
						  opt,S.zx,S.zy,S.zz,S.idx);
	% Integrator
	SI = S;
	iperm = @(u) u(SI.iperm,:);
	int_fcn = @(F) iperm(SE_fg_int_split_kaiser_mex_2p(0,F, ...
                                         opt,SI.zx,SI.zy,SI.zz,SI.idx));
else
	pre_t = tic;
	S = se2p_precomp(x,opt);
	walltime.pre = toc(pre_t);
	grid_fcn = @(f) SE_fg_grid_split_mex_2p(x(S.perm,:),f(S.perm), ...
						  opt,S.zs,S.zx,S.zy,S.zz,S.idx);
	% Integrator
	SI = S;
	iperm = @(u) u(SI.iperm,:);
	int_fcn = @(F) iperm(SE_fg_int_split_mex_2p(0,F, ...
						 opt,SI.zs,SI.zx,SI.zy,SI.zz,SI.idx));
end

% === Uncomment for direct code
if use_kaiser
    %grid_fcn = @(f) SE_fg_grid_kaiser_mex_2p(x,f,opt);
    %int_fcn = @(f) SE_fg_int_kaiser_mex_2p(x,f,opt);
else
...
scale_t = tic;
if use_kaiser
	[G, Gr, G0] = se2p_k_scaling_kaiser(H, Hr, H0, opt, pre_window);
else
	[G, Gr, G0] = se2p_k_scaling(H, Hr, H0, opt);
end
...
for i=1:dim_out
    fft_t = tic;
    F{i} = ifftnd2p(G{i}, Gr{i}, G0{i}, opt);
    walltime.fft = walltime.fft + toc(fft_t);
end
