...
void inline copy_in_real(double G[3][3], const mxComplexDouble* restrict g[3][3], size_t idx)
{
    for (int i=0; i<3; i++)
        for (int j=0; j<3; j++)
            G[i][j] = g[i][j][idx].real;
}
void inline copy_in_imag(double G[3][3], const mxComplexDouble* restrict g[3][3], size_t idx)
{
    for (int i=0; i<3; i++)
        for (int j=0; j<3; j++)
            G[i][j] = g[i][j][idx].imag;
}
void inline copy_out_real(double H[3], mxComplexDouble* restrict h[3], size_t idx)
{
    for (int i=0; i<3; i++)
        h[i][idx].real = H[i];
}
void inline copy_out_imag(double H[3], mxComplexDouble* restrict h[3], size_t idx)
{
    for (int i=0; i<3; i++)
        h[i][idx].imag = H[i];
}
		 ...
		 int nrhs, const mxArray *prhs[] )
{
    ...
    const mxComplexDouble* restrict g[3][3];
    for (int i=0; i<3; i++)
	for (int j=0; j<3; j++)
	{
	    ...
	    g[i][j] = mxGetComplexDoubles(cell);
	}
#else
...
#if MX_HAS_INTERLEAVED_COMPLEX
    double* restrict h[3];
    h[0] = mxGetComplexDoubles(H1);
    h[1] = mxGetComplexDoubles(H2);
    h[2] = mxGetComplexDoubles(H3);
#else
...
#if MX_HAS_INTERLEAVED_COMPLEX
		// real part
		copy_in_imag(G, g, idx);
		op_tensor(H, G, k, k2, B);
		copy_out_real(H, h, idx);
		// imaginary part
		copy_in_real(G, g, idx);
		op_tensor(H, G, k, k2, -B);
		copy_out_imag(H, h, idx);
#else
		// real part
		copy_in(G, gi, idx);
		op_tensor(H, G, k, k2, B);
