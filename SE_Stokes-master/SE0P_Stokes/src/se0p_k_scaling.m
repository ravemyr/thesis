function G = se0p_k_scaling(H, se0p_opt, opt, pre)

G = cell(3, 1);
GR = -pre.GR; % why minus? because the paper says so!

% MEX
[G{1} G{2} G{3}] = SE0P_stokeslet_fast_fs_k_scaling(H{1}, H{2}, H{3}, GR, opt.xi, ...
						   se0p_opt.padded_box, opt.eta);

% % Pure MATLAB:
% % k-vectors
% [k1 k2 k3] = k_vectors(se0p_opt.padded_M, se0p_opt.padded_box);
