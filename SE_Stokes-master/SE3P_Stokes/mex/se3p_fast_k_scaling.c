		 ...
		 int nrhs, const mxArray *prhs[] )
{
    ...
    /* Code for MATLAB versions from R2018a and onward */
    // Pointers to input arrays
    const mxComplexDouble* restrict g1 = mxGetComplexDoubles(G1);
    const mxComplexDouble* restrict g2 = mxGetComplexDoubles(G2);
    const mxComplexDouble* restrict g3 = mxGetComplexDoubles(G3);

    // Create the complex output arrays
    ...
    mxComplexDouble* h1 = mxGetComplexDoubles(H1);
    mxComplexDouble* h2 = mxGetComplexDoubles(H2);
    mxComplexDouble* h3 = mxGetComplexDoubles(H3);
	...
	for(i1 = 0; i1<N1; i1++)
	{
		...
		if(i0 != k0max || i1 != k1max || i2 != k2max)       // exclude k=0
		{
		    ...
		    h1[idx].real = q*( B[0][0]*G[0]+B[0][1]*G[1]+B[0][2]*G[2] );
		    h2[idx].real = q*( B[1][0]*G[0]+B[1][1]*G[1]+B[1][2]*G[2] );
		    h3[idx].real = q*( B[2][0]*G[0]+B[2][1]*G[1]+B[2][2]*G[2] );
		    ...
		    h1[idx].imag = q*( B[0][0]*G[0]+B[0][1]*G[1]+B[0][2]*G[2] );
		    h2[idx].imag = q*( B[1][0]*G[0]+B[1][1]*G[1]+B[1][2]*G[2] );
		    h3[idx].imag = q*( B[2][0]*G[0]+B[2][1]*G[1]+B[2][2]*G[2] );

		}
		else // don't assume initalized to zero
		{
		    h1[idx].real = 0; h2[idx].real = 0; h3[idx].real = 0;
		    h1[idx].imag = 0; h2[idx].imag = 0; h3[idx].imag = 0;
		}
	    }
	}
    }
#else
    /* Code for MATLAB versions up to R2017b */
